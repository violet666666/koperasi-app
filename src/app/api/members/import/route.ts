                    ...
                    await prisma.$transaction(async (tx) => {
                        ...
                        if (!pokokAcc && pokok > 0 && globalPProd) {
                            pokokAcc = await tx.savingsAccount.create({
                                data: { memberId: member.id, productId: globalPProd.id, balance: 0, status: "active", accountNo: `PKK-${member.memberNo || member.id}-${Date.now()}` },
                                include: { product: true }
                            });
                        }
                        if (!wajibAcc && (wajibAwal > 0 || monthlyDeposits.length > 0) && globalWProd) {
                            wajibAcc = await tx.savingsAccount.create({
                                data: { memberId: member.id, productId: globalWProd.id, balance: 0, status: "active", accountNo: `WJB-${member.memberNo || member.id}-${Date.now()}` },
                                include: { product: true }
                            });
                        }
                        if (!sukarelaAcc && sukarelaAwal > 0 && globalSProd) {
                            sukarelaAcc = await tx.savingsAccount.create({
                                data: { memberId: member.id, productId: globalSProd.id, balance: 0, status: "active", accountNo: `SKR-${member.memberNo || member.id}-${Date.now()}` },
                                include: { product: true }
                            });
                        }
