...
export async function DELETE(
    ...
    try {
        ...
        // Must ensure we only delete NON_SP_OUT
        const journal = await prisma.journal.findUnique({
            where: { id: journalId },
        });

        if (!journal || journal.sourceType !== "NON_SP_OUT") {
            return NextResponse.json({ message: "Pengeluaran tidak valid" }, { status: 404 });
        }
        ...
        console.error("DELETE /api/non-sp/pengeluaran/[id] error:", error);
