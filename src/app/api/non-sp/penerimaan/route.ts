...
export async function GET(request: Request) {
    try {
        ...
        const data = journals.map((j) => {
            // Find the income line and the asset line
            const incomeLine = j.lines.find(l => l.account.type === 'income');
            ...
            return {
                ...
                category: incomeLine?.account.name || "Penerimaan Lain-lain",
                description: j.description,
                amount: Number(incomeLine?.credit || 0),
        ...
        console.error("GET /api/non-sp/penerimaan error:", error);
        return NextResponse.json({ message: "Failed to fetch penerimaan" }, { status: 500 });
    }
}
...
export async function POST(request: Request) {
    try {
        ...
        const journalNo = `NSP-IN-${currentYear}-${random}`;
        ...
        const result = await prisma.$transaction(async (tx) => {
            const journal = await tx.journal.create({
                data: {
                    ...
                    branchId,
                    transactionDate,
                    description: description || "Penerimaan Non-S/P",
                    sourceType: "NON_SP_IN",
                    periodId: period.id,
                    isPosted: true,
                    createdById: userId,
            ...
            await tx.journalLine.create({
                data: {
                    ...
                    description: description || "Pendapatan Non-SP",
                    debit: 0,
                    credit: amountNum,
                },
            });
        ...
        console.error("POST /api/non-sp/penerimaan error:", error);
        return NextResponse.json({ message: "Failed to create penerimaan" }, { status: 500 });
    }
}
