...
// DELETE /api/unit/[slug]/operational-income/[id]
export async function DELETE(
    ...
    try {
        ...
        if (!access.authorized) {
            return NextResponse.json({ message: "Hanya Admin Unit atau Operator yang dapat menghapus pemasukan." }, { status: 403 });
        }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            // Deleting "in" (income) = subtract amount (-).
        ...
        if (deletedRecord && access.session) {
            logAuditFromRequest(request, access.session, {
                action: "DELETE",
                module: "Laporan",
                description: `Hapus pemasukan operasional ${deletedRecord.unitType || ""}: ${deletedRecord.description?.split("||RECEIPT:")[0] || "-"} — Rp${Number(deletedRecord.amount).toLocaleString("id-ID")}. Alasan: ${reason}`,
        ...
        return NextResponse.json({ message: "Pemasukan berhasil dihapus dan saldo kas/bank terkalkulasi ulang" });
    } catch (error: any) {
        console.error("DELETE Ops Income Error:", error);
        return NextResponse.json({ message: error.message || "Gagal menghapus pemasukan." }, { status: 500 });
    }
}

// PUT /api/unit/[slug]/operational-income/[id]
export async function PUT(
    ...
    try {
        ...
        if (!access.authorized) {
            return NextResponse.json({ message: "Hanya Admin Unit atau Operator yang dapat mengubah pemasukan." }, { status: 403 });
        }
        ...
        let keepExistingReceipt = formData.get("keepExistingReceipt") === "true";
        ...
        await prisma.$transaction(async (tx) => {
            ...
            // Because this is an IN payment, its net impact was +oldAmount
            ...
            const descWithMeta = receiptImagePath
                ? `[${(unitType as string).toUpperCase()}] Pemasukan Operasional: ${descriptionRaw}||RECEIPT:${receiptImagePath}`
                : `[${(unitType as string).toUpperCase()}] Pemasukan Operasional: ${descriptionRaw}`;
        ...
        return NextResponse.json({ message: "Pemasukan operasional berhasil diperbarui." });
    } catch (error: any) {
        console.error("PUT Ops Income Error:", error);
