...
import { buildProductProfit, ProductProfitRow } from "@/lib/services/product-profit";
...
export interface UnitLaporanResult {
  ...
  /** Laba per produk (harga jual − HPP). Kosong untuk unit tanpa StoreSale. */
...
export async function getUnitLaporanData(
  ...
  return {
    ...
    productProfit: usesStoreSales ? buildProductProfit(storeSales) : [],
    pagination: {
