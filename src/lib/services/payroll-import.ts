...
// File constants (used by both web + mobile routes for validation)
export const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10MB
export const ALLOWED_EXTENSIONS = [".xlsx", ".xls", ".csv"];
export const ALLOWED_SOURCES = ["polres", "polsek"];
export const DEFAULT_SISA_REKENING = 100_000; // minimum balance retained in BRI account
...
export type ParsedSlip = SlipData;
...
export interface ParsePayrollExcelResult {
    rows: ParsedSlip[];
    periodInfo: PayrollPeriodInfo;
    counts: { totalRows: number; success: number; failed: number };
    headers: string[];
    sheetName: string;
    columnCount: number;
}

export interface CommitPayrollPeriodResult {
    periodId: number;
    periodName: string;
    counts: { totalRows: number; success: number; failed: number };
}
...
/**
 * Typed Error subclass carrying an HTTP status code + status message, so routes
 * can map helper failures back to the exact response the original web route returned
 * (e.g. 400 for missing sheet, 409 for duplicate period).
 */
...
/**
 * Parse an Excel/CSV payroll file buffer into structured slips + period info.
 *
 * FAITHFUL VERBATIM move of the web route's parse block (route.ts ~lines 146-309):
 * same XLSX.read, sheet-find ("POT GAJI" / "POTONGAN"), sheet_to_json options,
 * header-row detect (first 10 rows, NRP/NIP + NAMA), 4-priority keyword column-map
 * (SUMMARY → KOPERASI → IDENTITY → BRI/other), per-row extract, member match
 * (exact nrp/memberNo → fuzzy name), tunkin from member.tunlesKinerja, period parse
 * (Indonesian month name + 4-digit year from filename).
 *
 * The only Prisma call is the member-match query (load all non-deleted members).
 */
export async function parsePayrollExcel(
    fileBuffer: Buffer,
    sourceType: string,
    sourceFileName: string,
): Promise<ParsePayrollExcelResult> {
    ...
    if (!sheetName) {
        throw new PayrollImportError(400, "Sheet 'POT GAJI' tidak ditemukan dalam file");
    }
    ...
    if (rows.length < 3) {
        throw new PayrollImportError(400, "Sheet kosong");
    }
    ...
    if (headerRowIdx === -1) {
        throw new PayrollImportError(400, "Header row tidak ditemukan. Pastikan sheet memiliki kolom NRP dan NAMA.");
    }
    ...
    return {
        rows: slips,
        periodInfo: { periodName, periodMonth, periodYear },
        counts: { totalRows: slips.length, success: slips.length, failed: skippedRows.length },
...
/**
 * Commit a parsed payroll period to the DB in a single transaction.
 *
 * FAITHFUL VERBATIM move of the web route's commit block (route.ts ~lines 347-414):
 * duplicate check on [periodMonth, periodYear, sourceType] (throws PayrollImportError(409)),
 * aggregate totals, create PayrollPeriod (status "processed", createdById=userId,
 * sourceFile=sourceFileName), batch payrollSlip.createMany (chunks of 100), per-slip
 * member.update({ data: { sisaGaji } }) where memberId && sisaGaji > 0.
 */
export async function commitPayrollPeriod(
    rows: ParsedSlip[],
    periodInfo: PayrollPeriodInfo,
    sourceType: string,
    sourceFileName: string,
    userId: number,
): Promise<CommitPayrollPeriodResult> {
    const { periodName, periodMonth, periodYear } = periodInfo;
    ...
    const period = await prisma.$transaction(async (tx) => {
        const p = await tx.payrollPeriod.create({
            data: {
                ...
                totalGaji: rows.reduce((sum, s) => sum + s.gajiBersih, 0),
                totalPotongan: rows.reduce((sum, s) => sum + s.totalPotKoperasi, 0),
                createdById: userId,
            },
        });
    ...
    return {
        periodId: period.id,
        periodName,
        counts: { totalRows: rows.length, success: rows.length, failed: 0 },
    };
}
