# Cafe LSP Unit — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add new unit "Cafe LSP" (`cafe_lsp`) with dedicated counter-based POS, order queue board, quick keys, auto queue numbers, and full admin CRUD.

**Architecture:** Dedicated POS page at `/cafe-lsp/kasir` using StoreSale/StoreProduct infrastructure (same as Toko and Resto). Wrapper pages reuse Toko components. No Prisma schema changes needed — `cafe_lsp` is a String unitType.

**Tech Stack:** Next.js App Router, React, Zustand (state), Tailwind, shadcn/ui, Prisma, SQLite
...
|---|---|---|
| CREATE | `src/app/(protected)/cafe-lsp/kasir/page.tsx` | Dedicated counter-based POS |
| CREATE | `src/app/(protected)/cafe-lsp/antrian/page.tsx` | Order Queue Board |
| CREATE | `src/app/(protected)/cafe-lsp/produk/page.tsx` | Wrapper → TokoProdukPage |
| CREATE | `src/app/(protected)/cafe-lsp/shift/page.tsx` | Wrapper → TokoShiftPage |
| CREATE | `src/app/(protected)/cafe-lsp/marketing/page.tsx` | Wrapper → TokoMarketingPage |
| CREATE | `src/app/(protected)/cafe-lsp/persediaan/page.tsx` | Wrapper → TokoPersediaanPage |
| MODIFY | `src/lib/constants/navigation.ts` | Add kasirCafeLspNavigation + adminCafeLspNavigation + routing |
| MODIFY | `src/app/(protected)/layout.tsx` | Add cafe_lsp to route guard |
| MODIFY | `src/app/api/users/route.ts` | Add "cafe_lsp" to VALID_UNIT_TYPES |
| MODIFY | `src/lib/validations/index.ts` | Add "cafe_lsp" to unitType enum |
| MODIFY | `prisma/seed.ts` | Add Cafe LSP staff accounts |
| MODIFY | `src/components/patterns/kasir-dashboard.tsx` | Add cafe_lsp POS route |

---

## Task 1: Validation Whitelist & Seed Script

**Files:**
- Modify: `src/app/api/users/route.ts:9`
- Modify: `src/lib/validations/index.ts:232-235`
- Modify: `prisma/seed.ts:290-301`
- Modify: `prisma/schema.prisma` (comment only)

- [ ] **Step 1: Add `cafe_lsp` to VALID_UNIT_TYPES**

In `src/app/api/users/route.ts` line 9, change:

```typescript
const VALID_UNIT_TYPES = ["toko", "resto_cafe", "barbershop", "cuci_mobil", "fitness", "playstation", "laundry", "fotocopy", "simpan_pinjam", "cafe_lsp"];
```

- [ ] **Step 2: Add `cafe_lsp` to validation enum**

In `src/lib/validations/index.ts` lines 232-235, change:
...
- [ ] **Step 3: Add Cafe LSP to seed script**

In `prisma/seed.ts`, after line 299 (barbershop entry), before line 300 (aset entry), add:
...
This will create:
- `admincafelsp@koperasi.com` / `password123` (role: admin, unitType: cafe_lsp)
- `kasircafelsp@koperasi.com` / `password123` (role: kasir, unitType: cafe_lsp)

- [ ] **Step 4: Update Prisma schema comment**

In `prisma/schema.prisma`, find the `unitType` field in User model (around line 26). Update the comment to include `cafe_lsp`:

```prisma
  unitType        String?   @map("unit_type") // simpan_pinjam, toko, fitness, cuci_mobil, fotocopy, laundry, resto_cafe, playstation, barbershop, aset, cafe_lsp
```

- [ ] **Step 5: Commit**

```bash
git add src/app/api/users/route.ts src/lib/validations/index.ts prisma/seed.ts prisma/schema.prisma
git commit -m "feat: add cafe_lsp unitType to validation whitelist and seed script"
```

---

## Task 2: Navigation Arrays

**Files:**
- Modify: `src/lib/constants/navigation.ts`

- [ ] **Step 1: Add kasirCafeLspNavigation**

In `src/lib/constants/navigation.ts`, after the `kasirLaundryNavigation` block (around line 857), add:
...
- [ ] **Step 2: Add adminCafeLspNavigation**

After the `kasirCafeLspNavigation` block, add:
...
- [ ] **Step 3: Add cafe_lsp routing in getNavigationForUser**

In `getNavigationForUser` function, add `cafe_lsp` routing. After the kasir laundry check (line ~1037) and before the kasir fallback (line ~1040), add:
...
After the admin laundry check (line ~1063) and before the admin jasa fallback (line ~1066), add:
...
Also update the admin jasa fallback exclusion list at line 1066 to include `cafe_lsp`:
...
git commit -m "feat: add kasirCafeLspNavigation and adminCafeLspNavigation"
```
...
- [ ] **Step 1: Add cafe_lsp to KASIR_ALLOWED_ROUTES**

In `src/app/(protected)/layout.tsx`, after the `resto_cafe` entry (line 24), add:

```typescript
    cafe_lsp:    ["/unit", "/transaksi-unit", "/cafe-lsp"],
```

So `KASIR_ALLOWED_ROUTES` becomes:
...
- [ ] **Step 2: Add cafe_lsp to ADMIN_ALLOWED_ROUTES**

After the `resto_cafe` admin entry (line 47), add:

```typescript
    cafe_lsp:    ["/unit", "/transaksi-unit", "/toko", "/cafe-lsp", "/approval"],
```

So `ADMIN_ALLOWED_ROUTES` becomes (relevant section):
...
git commit -m "feat: add cafe_lsp route guard for kasir and admin"
```

---

## Task 4: Kasir Dashboard POS Route

**Files:**
- Modify: `src/components/patterns/kasir-dashboard.tsx`

- [ ] **Step 1: Add cafe_lsp to DEDICATED_POS_ROUTES**

In `src/components/patterns/kasir-dashboard.tsx`, after line 69 (`resto: "/resto/kasir",`), add:
...
```bash
git add src/components/patterns/kasir-dashboard.tsx
git commit -m "feat: add cafe_lsp POS route to kasir dashboard"
```

---

## Task 5: Wrapper Pages (produk, shift, marketing, persediaan)

**Files:**
- Create: `src/app/(protected)/cafe-lsp/produk/page.tsx`
- Create: `src/app/(protected)/cafe-lsp/shift/page.tsx`
- Create: `src/app/(protected)/cafe-lsp/marketing/page.tsx`
- Create: `src/app/(protected)/cafe-lsp/persediaan/page.tsx`

- [ ] **Step 1: Create produk wrapper**
...
export default function CafeLspShiftPage() {
    return <TokoShiftPage />;
}
```

- [ ] **Step 3: Create marketing wrapper**

Create `src/app/(protected)/cafe-lsp/marketing/page.tsx`:

```typescript
import TokoMarketingPage from "@/app/(protected)/toko/marketing/page";

export default function CafeLspMarketingPage() {
    return <TokoMarketingPage />;
}
```

- [ ] **Step 4: Create persediaan wrapper**
...
```bash
git add src/app/\(protected\)/cafe-lsp/produk/page.tsx src/app/\(protected\)/cafe-lsp/shift/page.tsx src/app/\(protected\)/cafe-lsp/marketing/page.tsx src/app/\(protected\)/cafe-lsp/persediaan/page.tsx
git commit -m "feat: add cafe-lsp wrapper pages (produk, shift, marketing, persediaan)"
```

---

## Task 6: Dedicated POS Page — Cafe LSP Kasir

**Files:**
- Create: `src/app/(protected)/cafe-lsp/kasir/page.tsx`

This is the largest task. The POS is adapted from Resto's POS but simplified:
- **No table management** — single cart, counter-based
- **Queue number** — auto-increment per day
- **Quick Keys tab** — best sellers
- **Order Queue panel** — bottom strip showing active orders
- **Shift validation** — lock checkout if no shift, link to `/cafe-lsp/shift`

- [ ] **Step 1: Create the POS page**

Create `src/app/(protected)/cafe-lsp/kasir/page.tsx` with the full counter-based POS. This file is based on the Resto POS pattern but replaces table management with a single cart + queue number system.

Key differences from Resto POS:
1. No `RestoTable` / `useRestoStore` — uses simple `cart` state instead
2. Auto-generates queue number (A001, A002...) from API count
3. Quick Keys tab shows best sellers or manually flagged items
4. Order Queue panel at bottom shows pending orders
5. Shift check uses `unitType=cafe_lsp`
6. All API calls use `unitType=cafe_lsp`
...
import { Coffee, Search, Banknote, CreditCard, Loader2, Maximize, ShieldAlert, ShieldCheck, User, Trash2, Plus, Minus, LayoutGrid, ImageOff, AlertCircle, CheckCircle2, QrCode, Star, Clock, ListOrdered, UtensilsCrossed } from "lucide-react";
...
export default function CafeLspKasirPage() {
    ...
    // Quick Keys — derive from products flagged as quick key or top sellers
    const quickKeyProducts = React.useMemo(() => {
        ...
        // Fallback: first product from each category
    ...
    // Shift check — with unitType filter
    React.useEffect(() => {
    ...
    React.useEffect(() => {
        ...
        fetchQueueCount();
    }, [cart.length === 0]); // refresh when cart empties (after sale)
...
Expected: No TypeScript errors related to cafe-lsp/kasir/page.tsx

- [ ] **Step 3: Commit**

```bash
git add "src/app/(protected)/cafe-lsp/kasir/page.tsx"
git commit -m "feat: add Cafe LSP dedicated counter-based POS page"
```

---

## Task 7: Order Queue Board Page

**Files:**
- Create: `src/app/(protected)/cafe-lsp/antrian/page.tsx`

This is a full-screen display page for a separate monitor showing all pending orders with queue numbers. It polls the sales API to show recent orders.

- [ ] **Step 1: Create the Order Queue Board**
...
export default function CafeLspAntrianPage() {
    ...
    const fetchOrders = async () => {
        try {
            const today = new Date();
            const start = new Date(today.getFullYear(), today.getMonth(), today.getDate()).toISOString();
            const res = await fetch(`/api/toko/sales?unitType=cafe_lsp&from=${start}&limit=50`);
            const json = await res.json();
    ...
    // Keep readyIds in sync
    React.useEffect(() => {
        // Persist readyIds to localStorage
...
```bash
git add "src/app/(protected)/cafe-lsp/antrian/page.tsx"
git commit -m "feat: add Cafe LSP Order Queue Board page"
```

---

## Task 8: Seed Database & Verify

**Files:**
- No new files — run seed script
...
Expected: Creates `admincafelsp@koperasi.com` and `kasircafelsp@koperasi.com` users with `unitType=cafe_lsp`.

Note: If seed script fails because other users already exist, the `cafe_lsp` entry will still be created since it processes each entry independently.
...
Or query the database to verify:
- User with email `admincafelsp@koperasi.com` has `unitType=cafe_lsp`, `roleId` = admin
- User with email `kasircafelsp@koperasi.com` has `unitType=cafe_lsp`, `roleId` = kasir
...
Test:
1. Login as `kasircafelsp@koperasi.com` / `password123`
2. Verify sidebar shows Cafe LSP navigation (Kasir POS, Order Queue, Shift, Riwayat)
3. Navigate to `/cafe-lsp/kasir` — POS should load with empty menu
4. Login as `admincafelsp@koperasi.com` / `password123`
5. Verify sidebar shows full admin navigation (Manajemen Menu, Promo, Stok, Laporan)
6. Navigate to `/cafe-lsp/produk` — should load Toko Produk page with "Manajemen Menu" label
...
```bash
git add -A
git commit -m "fix: minor fixes for Cafe LSP integration"
```
...
**Files:**
- Create: `UNIT-CAFE-LSP.md`

- [ ] **Step 1: Create documentation file**

Create `UNIT-CAFE-LSP.md` in the project root:
...
| Order Queue Panel | `cafe-lsp/kasir/page.tsx` | ✅ |
...
```bash
git add UNIT-CAFE-LSP.md
git commit -m "docs: add Cafe LSP unit documentation"
```
...
- [x] **Spec coverage:** Each section of the spec maps to a task:
  - Validation whitelist → Task 1
  - Navigation → Task 2
  - Route guard → Task 3
  - Dashboard POS route → Task 4
  - Wrapper pages → Task 5
  - Dedicated POS → Task 6
  - Order Queue Board → Task 7
  - Seed & verify → Task 8
  - Documentation → Task 9
- [x] **No placeholders:** Every step has actual code or exact commands
- [x] **Type consistency:** All interfaces (Product, CartItem, etc.) are consistent across tasks
- [x] **unitType consistency:** `cafe_lsp` used consistently in all files
