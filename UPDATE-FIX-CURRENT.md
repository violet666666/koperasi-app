...
### 12 April 2026 - Peningkatan UI/UX Form Tambah Simpanan
1. **Penyembunyian Field Kas Koperasi pada Transaksi Setoran**: Memperbaiki alur UX sesuai kebiasaan operator dengan menyembunyikan "Metode Pembayaran" dan "Kas Koperasi" ketika memilih jenis transaksi Setoran (deposit). Field-field tersebut hanya akan dirender ketika operator benar-benar butuh mengeluarkan uang dari kas tunai (Penarikan).
2. **Reorganisasi Logika *Layout* Form**: Mengubah urutan card pada halaman form dari yang tadinya (Anggota -> Jenis -> Produk) menjadi **(Anggota -> Produk -> Jenis -> Detail)**. Alur ini secara logis jauh lebih benar karena *Pemilihan Produk* di awal akan langsung menentukan secara cerdas (sistematis) apakah *Jenis Transaksi Penarikan* boleh di-klik atau harus di-disable (diblokir oleh AD-ART seperti kasus penarikan Simpanan Wajib).
